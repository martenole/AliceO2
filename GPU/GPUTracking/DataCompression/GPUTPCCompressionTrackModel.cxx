...
GPUd() void GPUTPCCompressionTrackModel::Init(float x, float y, float alpha, unsigned char qPt, const GPUParam& GPUrestrict() param)
{
  ...
  mNDF = -5;
  mParam = &param;
  mField = &mParam->polynomialField;
  //calculateMaterialCorrection(); // TODO
}
...
GPUd() int GPUTPCCompressionTrackModel::Filter(float y, float z, int iRow)
{
  ...
  float err2Y, err2Z;
  mParam->GetClusterRMS2(iRow, z, mTrk.sinphi, mTrk.dzds, err2Y, err2Z);
  mParam->UpdateClusterError2ByState(0, err2Y, err2Z);
  if (mNDF == -5) {
...
GPUd() int GPUTPCCompressionTrackModel::rotateToAlpha(float newAlpha)
{
  ...
  if (CAMath::Abs(mTrk.sinphi) >= GPUCA_MAX_SIN_PHI || CAMath::Abs(px0) < (1 - GPUCA_MAX_SIN_PHI)) {
    return -1;
  }
...
GPUd() int GPUTPCCompressionTrackModel::followLinearization(const PhysicalTrackModel& t0e, float Bz, float dLp)
{
  ...
  /*
  // ignore material correction for now
  if (mMatLUT) {
    UpdateMaterial(t0e);
  }
  */
